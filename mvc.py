...
class MVC:
    ...
    def getuplRatio_instId(api_key, secret_key, passphrase, flag):
        ...
        if len(datas) > 0:
            ...
            for item in datas:
                ...
                if uplRatio > 0.5 or uplRatio < -30:
    ...
    def orderbuy(api_key, secret_key, passphrase, flag, symbol, minute):
        ...
        if upl + cashBal * 0.8 < 0:
            ...
            return False
        elif float(details["mgnRatio"]) * 100 < 500:
            print("账户保证金率低于500%")
            return False
        elif float(details["frozenBal"]) > cashBal * 0.8:
            print("保证金大于" + str(cashBal * 0.8) + "美金＄")
            return False
        elif (len(posData) < 50 and minute != "low") or minute == "low" or minute == "imr":
